...
技能不依赖聊天窗口的图片粘贴——它通过**文件路径**读取图片/视频。三种客户端模式：

| 模式 | 表现 | 用法 |
|---|---|---|
| 路径注入（如 ZCode 桌面端） | 粘贴的图片自动保存到本地并给主模型路径 | 直接粘贴即可，技能自动读取 |
| 附件/文件通道（多数客户端） | 粘贴图片被拦，但附件发送可用 | 用"附件/文件"方式发送图片，技能从附件路径读取 |
| 完全拦截（如 WorkBuddy 手机端 + 纯文本模型） | 客户端报"当前模型暂不支持图片" | 保存图片到本地，提供文件路径给技能 |

技能会在能力自检时探测当前客户端属于哪种模式，并按 `references/input.md` 自动适配——无法直接拿到图片时，会明确引导你走可用的通道，绝不假装收到。
