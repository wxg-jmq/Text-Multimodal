...
一套开箱即用的 **SKILL**：任何文本模型助手（Claude / ChatGPT / Gemini / Cursor / Copilot / 本地模型……）加载后，即可从零构建完整的多模态能力体系——**看图、识视频、生成图片、生成视频**，且所有结果尽量在聊天窗口中内联预览。
...
技能不依赖聊天窗口的图片粘贴——有**四条通道**可以拿到图片/视频，总有一条通：

| 通道 | 适用 | 用法 |
|---|---|---|
| 路径注入 | 粘贴的图片自动存本地并给主模型路径（如 ZCode 桌面端） | 直接粘贴即可，技能自动读取 |
| 附件/文件 | 粘贴被拦，但附件发送可用（多数客户端） | 用"附件/文件"方式发送，技能从附件路径读取 |
| 剪贴板提取 | 桌面端 + 有终端：粘贴被拦但图在系统剪贴板 | 粘贴后直接说"识别这张图"，技能自动用 `scripts/clipboard_save.py` 提取 |
| URL 直传 | 任何客户端、任何纯文本模型（全平台兜底） | 把图片/视频传到可访问的 URL，贴链接即可（链接是文本，不拦截） |

技能会在能力自检时探测当前环境有哪些通道可用，并按 `references/input.md` 自动适配——手机端（无终端、剪贴板不可读）自动走附件/URL；视频自动跳过剪贴板通道。无法拿到时明确引导，绝不假装收到。
...
技能本质是一个目录：`text-multimodal/`（内含 `SKILL.md` + `references/` + `scripts/` + `assets/`，即克隆后仓库里的 `text-multimodal/` 文件夹）。安装 = 把这个目录放到你的 Agent 能发现技能的位置，或把 `SKILL.md` 全文发给助手。以下方法任选其一。
